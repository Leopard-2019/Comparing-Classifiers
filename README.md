...
The Business goal is  to come up which a machine learning classification model to identify if a future client will subscribe a term deposit or not based on several independent factors such as education level, marital status, if has housing loan, personal loan, etc. The machine learning classification model is selected by ranking up four different models (KNeighborsClassifier, Logistic Regression, Support Vector Machine, and Decision Tree) by their metrics and other indicators such as and other indicator such as the Precision-recal curve, and confusion matrix. The dataset used to train those four models is related to the marketing of bank products over the telephone as mentioned before. The analysis will be done using python & jupyter notebook.
